...
## LPS22HBのNotifyが届かない問題（解決済み, 2026-08-17）

### 調査

`sensor_lps22hb.go`に一時的にログを追加し（RTTバッファ直読み手法を再利用）、`readLPS22HB()`の`STATUS`レジスタ(0x27)ポーリングを観察したところ、**毎回`status=0x00`のまま**で、T_DA/P_DAビットが一度も立たないことが判明。元のループにはタイムアウトが無かったため、goroutineが実質無限ループでブロックし続け、一度もNotifyが送られていなかった。

### 根本原因

`i2c.go`の`regWrite`/`regRead`には**排他制御が一切無かった**。LPS22HB/HDC2010/BMX055はそれぞれ独立したgoroutineから同じI2Cバス（`machine.I2C0`）に周期的にアクセスしており、`machine.I2C.Tx`自体は並行呼び出しに対して安全である保証がない。複数センサーのI2Cトランザクションが同時に競合し、LPS22HBの「ワンショット計測開始→ステータスポーリング→データ読み出し」という一連の手順が他センサーのアクセスと衝突して壊れていたと考えられる（HDC2010/BMX055がconfigureMagTrim等の連続レジスタアクセスパターン上、たまたま影響を受けにくかっただけで、LPS22HBだけが症状として顕在化した可能性が高い）。

### 対処

`i2c.go`に`sync.Mutex`を追加し、`regWrite`/`regRead`それぞれの呼び出し単位でロックするよう変更。加えて、元々無限ループだったstatusポーリングに最大20回(約200ms)のタイムアウトを追加し、仮にセンサーが応答しなくても該当goroutineが永久にブロックしないようにした。

**検証結果**: 修正後、LPS22HB/HDC2010/Accel/Gyro/Mag全5キャラクタリスティックが20秒間で均等に20回ずつNotifyを送信することを確認（以前はLPS22HBだけ0回だった）。

### 教訓

複数のgoroutineが同じハードウェアバス（I2C/SPI等）に個別にアクセスする設計では、バスドライバ自体に排他制御が無い前提で、呼び出し側で明示的にロックする必要がある。今回は正常に動いていたHDC2010/BMX055が「たまたま」動いていただけで、センサーが増えるごとに競合の顕在化リスクは上がる。ポーリングループには常にタイムアウトを入れ、無限にブロックしうる箇所を作らないことも合わせて重要。
