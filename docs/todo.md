...
- [x] ~~電源管理・スリープモードの検討~~（2026-08-17完了・実装済み）: ジャイロ(BMG160)が常時Normalモード(5mA)で支配的な電流消費要因だったため、`sensor_bmx055.go`に`readGyroDutyCycled()`を追加し、読み取り直前だけLPM1レジスタでNormalに起こし(データシート記載のwake-up time 30ms待機)、読み取り後は即Suspend(25µA)へ戻す方式に変更。加速度・磁気センサーは常時稼働のまま（今回は対象外、必要なら別途検討）。CR2032想定でジャイロ平均電流は約174µAまで低下（常時5mA時の約1.8日→全体で約8〜12日程度に改善見込み、磁気センサーとBLE/CPUベースラインが次の支配項目）。実機でNotify 20/20確認済み（`make verify`）
